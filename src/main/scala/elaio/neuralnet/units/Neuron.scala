...
import elaio.neuralnet.connections.{Connection, Parameter}
...
abstract class Neuron(val id: Long) {
  ...
  private var _bias: Parameter = new Parameter
  ...
  def biasParameter: Parameter = _bias
  def biasParameter_=(cell: Parameter): Unit = { _bias = cell }
